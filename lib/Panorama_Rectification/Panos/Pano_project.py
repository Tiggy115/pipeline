...
import glob
from lib.Panorama_Rectification.Panos.Pano_visualization import R_roll, R_pitch, R_heading
from lib.Panorama_Rectification.Panos.Pano_new_pano import calculate_new_pano, save_heading_pitch_json
from lib.Panorama_Rectification.Panos.Pano_refine_project import simon_refine
import json
...
def project_facade_for_refine(final_hvps_rectified, im, pitch, roll, im_path, root, tmp_folder, rendering_img_base,
                              tmp_count):
    no_adaptive = True
    if no_adaptive:
    ...
    for i in range(len(final_hvps_rectified)):
        ...
        for j in range(2):
            ...
            if no_adaptive:
                ...
                out = os.path.join(root, 'Pano_refine')
                if not os.path.isdir(out):
                    os.mkdir(out)
                save_path_main = os.path.join(out, name + '_VP_{}_{}.jpg'.format(i, j))
                ...
                if refine_radians != None:
                    ...
                    #  relax requirements
                    # [tmp_xy, m_tmp, n_tmp, _] = calculate_no_adaptive_coor(h_fov=140, v_fov1=-35, v_fov2=70,
                    #                                                       mpp=0.0125 * 2)
